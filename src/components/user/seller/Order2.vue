  ...
  <div>
    <div class="title">
      <ul>
        ...
        <li @click="ordersChange(1)" :class="{all:ordersSwitch==1}">待发货</li>
        <li @click="ordersChange(2)" :class="{all:ordersSwitch==2}">已完成</li>
<!--        <li @click="ordersChange(3)" :class="{all:ordersSwitch==3}">已完成</li>-->
    ...
    <div class="order-box" v-for="(order,index) in this.ordersShow" :key="index">
      <ol>
        <input type="checkbox" :value="order.id" v-model="deleteOrderList">
        <p>2021-5-22&nbsp;&nbsp;&nbsp;&nbsp;订单号：{{order.orderid}}</p>
        <a href="#"></a>
      </ol>
      <div class="order-content">
        <el-row>
          ...
          <el-col :md="5">
            <p>{{order.goodsName}}</p>
            <span><i style="color: #FD3F31; margin-right:7px" class="fa fa-user-circle" aria-hidden="true"></i>{{order.buyername}}</span>
          </el-col>
          <el-col :md="4">
          ...
          <el-col :md="6">
            <a v-show="order.issent===1" @click="confirm(order.orderid)" href="javascript:void(0)">确认发货</a>
            <a @click="detail(order.orderid)" href="javascript:void(0)">订单详情</a>
<!--            <a @click="drawback(order.id)" href="javascript:void(0)" class="text02">退款</a>-->
          </el-col>
        </el-row>
      </div>
    </div>
    <el-dialog title="查看订单内容" :visible.sync="checkDialogVisible" width="50%">
      <div>
        <el-form  label-position="right" label-width="70px" ref="formRef" :model="diagForm" >
          <el-form-item label="订单ID" >
            <el-input v-model="diagForm.orderid" disabled></el-input>
          </el-form-item>
          <el-form-item label="商品ID" >
            <el-input v-model="diagForm.gid" disabled></el-input>
          </el-form-item>
          <el-form-item label="商品名" >
            <el-input v-model="diagForm.gname" disabled></el-input>
          </el-form-item>
          <el-form-item label="买家id" >
            <el-input v-model="diagForm.buyerid" disabled></el-input>
          </el-form-item>
          <el-form-item label="价格" >
            <el-input v-model="diagForm.price" disabled></el-input>
          </el-form-item>
        </el-form>
      </div>
...
export default {
  ...
  methods: {
    ...
    async confirm (id) {
      const result = await this.$http.put('/order/beginDelivery', {
    ...
    detail (id) {
      ...
      this.$http.get('orders/' + id).then(res => {
        ...
        this.diagForm = res.data.data
        this.$http.get('orders/' + id).then(res2 => {
          this.diagForm.price = res2.data.data.price
          this.diagForm.gname = res2.data.data.gname
        })
        this.checkDialogVisible = true
      }).catch(err => {
        console.log(err)
